...
void drawButton2(NVGcontext* vg, int preicon, const char* text, float x, float y, float w, float h, NVGcolor col)
{
	NVGpaint bg,bg2;
	char icon[8];
	float cornerRadius = 4.0f;
    ...
    // Couleur de l'ombre
    NVGcolor shadowColor = nvgRGBA(0, 0, 0, 100);  // Couleur noire semi-transparente

    // Création du dégradé radial pour représenter l'ombre
    NVGpaint shadowPaint = nvgBoxGradient(vg, x, y, w, h, 100.0f, 200.0f, shadowColor, nvgRGBA(0, 0, 0, 0));

	bg = nvgLinearGradient(vg, x,y,x+0,y+h/1.5, nvgRGBA(0,0,230,255), nvgRGBA(0,0,100,255));
...
/*
	...
	if (preicon != 0)
	{
		...
		nvgFillColor(vg, nvgRGBA(255,255,255,96));
		nvgTextAlign(vg,NVG_ALIGN_LEFT|NVG_ALIGN_MIDDLE);
		nvgText(vg, x+w*0.5f-tw*0.5f-iw*0.75f-50, y+h*0.5f, cpToUTF8(preicon,icon), NULL);
	}
...
*/
	...
	nvgFillColor(vg, nvgRGBA(0,0,0,160));
	nvgText(vg, x+w*0.5f,y+h*0.5f-1,text, NULL);
...
void render_nanovg(int winWidth,int winHeight,float pxRatio,int iblowup,float it,float mx,float my)
{
        nvgBeginFrame(vg, winWidth, winHeight, pxRatio*1.0f);

        drawButton2(vg, ICON_LOGIN, "Sign in", 138, 20, 128, 32, nvgRGBA(0,96,128,255));

        //renderDemo(vg, mx,my, winWidth,winHeight, it, iblowup, &data);
        //drawWindow(vg,"test",0,0,300,200);
        //drawSearchBox(vg,"test",320,0,200,100);
