...
void getargs(const std::string &text)
{/*
...
void SUI_Widget::search(const std::string& text, const std::string& element,const int type)
{
    ...
    "brightness","brightness-color-linear","visible"
    ...
    for(int i = 0;i < key_style.size();i++)
    {
        ...
        if(args == "font-size")
            style.font_size = atof(arg);

        if(args == "font-color")
            style.font_color = hexToInt(args);

        if(args == "border-radius")
            style.border_radius = atof(arg);

        if(args == "background-color")
            style.background_color = hexToInt(args);

        if(args == "brightness")
            style.font_ttf = atof(arg);


        if(args == "font_shadow")
            style.font_shadow_x = atoi(arg);
